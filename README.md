...
- CSRF トークンの生成・検証機能
- セキュアなランダムトークン生成
- HTML form 用の hidden フィールド自動生成
- タイミング攻撃対策（hash_equals 使用）

```php
// 使用例
$token = CsrfHelper::getToken();
echo CsrfHelper::getHiddenInput(); // <input type="hidden" name="csrf_token" value="...">
if (CsrfHelper::verifyToken($token)) { /* 処理 */ }
```

#### Validator（バリデーション）

- メソッドチェーン対応のバリデーション機能
- 日本語エラーメッセージ対応
- 空白チェック、文字数制限、メール形式等の基本機能
...
- 全ビューデータの自動 XSS エスケープ
- CSRF トークンの自動追加
- バリデーター・JSON レスポンス機能統合

### 🛡️ LoginController セキュリティ強化
